...
import com.cyq.rpgmvtranshelper.utils.constant.ImageFileType;
 ...
 *
 */
public class ImageHandleUtils implements ImageFileType {
			...
			if (srcWidth > 0 && srcHeight > 0) {
				...
				for (int i = 0; i < rows; i++) {
					for (int j = 0; j < cols; j++) {
						...
						g.dispose();
						// 输出为文件
						ImageIO.write(tag, IMAGE_TYPE_JPEG, new File(descDir
								+ "_r" + i + "_c" + j + DOT_JPG));
					}
				}
			}
	...
	public final static void pressText(String pressText,
		...
		try {
			...
			g.dispose();
			ImageIO.write((BufferedImage) image, IMAGE_TYPE_JPEG, new File(destImageFile));// 输出到文件流
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	 ...
	 * @Author chengyiqun
	 * @param text
	 * @return standard Ascii length is 1, other is 2
	 */
	public final static int getLength(String text) {
		...
		for (int i = 0, strLen = text.length(); i < strLen; i++) {
			// 考虑扩展ascii码, 最大值是 0b1111_1111 也即是 ÿ
			if (((int) text.charAt(i) & 0xFFFFFFFF) < 0b1_0000_0000) {
				length += 1;
			} else {
				length += 2;
			}
...
//		return length / 2;
	...
	public static void main(String[] args) {
		System.out.println(getLength("程义群cc"));
	}
